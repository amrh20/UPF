...
declare var $j: any;
@Component({
  selector: 'app-datepicker-birth',
  templateUrl: './datepicker.component.html',
  styleUrls: ['./datepicker.component.css'],
  viewProviders: [{ provide: ControlContainer, useExisting: NgForm }]
})
export class DatepickerbirthComponent extends BaseComponent {
  inputValue: any;
  ...
  @Output() dateHijri : EventEmitter<any> = new EventEmitter();
  ...
  ngAfterViewInit() {
    ...
    altCommands.today.action = () => {
      ...
      // if (this.shownCalendar == 'hijri') {
    
      //   this.dateHijri.emit(newDate)
      // }
    }
    ...
    $j('#' + this.name + '_gregorian_datepicker_placeholder').calendarsPicker({
      ...
      maxDate: this.topDate, // The maximum selectable date
      ...
      onSelect: (dates) => {
        ...
        if (this.shownCalendar == 'hijri') {
          this.dateHijri.emit((hijri_date).formatDate('YYYY-mm-dd'));
        }
        else{
          this.dateHijri.emit(null);
    ...
    $j('#' + this.name + '_hijri_datepicker_placeholder').calendarsPicker({
      ...
      selectDefaultDate: false, // True to pre-select the default date if no other is chosen
      minDate: null, // The minimum selectable date
      maxDate:  this.topHijriDate, // The maximum selectable date
      dateFormat: 'dd/mm/yyyy', // Format for dates, defaults to calendar setting if null
      autoSize: false, // True to size the input field according to the date format
      rangeSelect: false, // Allows for selecting a date range on one date picker
      rangeSeparator: ' - ', // Text between two dates in a range
