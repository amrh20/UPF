...
@Component({
  selector: "app-national-address-section",
  templateUrl: "./national-address-section.component.html",
  styleUrls: ["./national-address-section.component.css"],
})
export class NationalAddressSectionComponent extends RequestDetailsSectionComponent {
  nationalAddressType
  nationalAddressIndex

  openPopUp(type) {
    this.section.body.details.nationalAddressSegment.nationalAddress.push(
      Object.assign({}, this.nationalAddressObj));
    let row = this.section.body.details.nationalAddressSegment.nationalAddress[this.section.body.details.nationalAddressSegment.nationalAddress.length - 1]
    let obj = {
      type: type,
      row: row,
      index: null
    }
    this.nationalAddressType = type
    this.nationalAddressIndex = null
  ...
  edit(type, index, row) {
    ...
    this.nationalAddressType = type
    this.nationalAddressIndex = index
    this._messageService.filter(obj);
  }

  close() {
    let ind
    if (this.nationalAddressIndex == null) {
      if (this.nationalAddressType == 'nationalAddressSegment') {
        ind = this.section.body.details['nationalAddressSegment']['nationalAddress'].length - 1
      }
    } else {
      ind = this.nationalAddressIndex
    }
    if (this.nationalAddressType == 'nationalAddressSegment') {
  ...
  formisvalid() {
    let obj, ind, note
    if (this.section.body.details.nationalAddressSegment && this.section.body.details.nationalAddressSegment.nationalAddress) {
      if (this.nationalAddressIndex == null) {
        ind = this.section.body.details['nationalAddressSegment']['nationalAddress'].length - 1
      } else {
        ind = this.nationalAddressIndex
      }
      obj = this.section.body.details.nationalAddressSegment.nationalAddress[ind]
    }
    if (obj) {
      ...
      if (obj && obj.buildingNumber
        && obj.streetName && obj.district
        && obj.city && obj.city.key
        && obj.postalCode && obj.additionalNumber
        && note
      ) {
        return true
      } else {
...
@Component({
  selector: 'app-national-address-popup',
  templateUrl: './national-address-details.component.html',
  styleUrls: ["./national-address-section.component.css"],
})
export class NationalAddressPopupDetailsComponent {
